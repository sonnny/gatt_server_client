...
static PT_THREAD (protothread_client(struct pt *pt))
{
    ...
    while(1) {
        ...
        printf("properties: %d\n", server_characteristic[counter].properties);
        printf("uuid16: %d\n", server_characteristic[counter].uuid16);
        for(int i=0; i<16; i++) printf("uuid128 %X ", server_characteristic[counter].uuid128[i]);
        printf("\n");
...
static PT_THREAD (protothread_ui(struct pt *pt))
{
   ...
   //////////////////////////////////////////////////// testing
            ...
            if (server_characteristic[temp_var].properties & (1u<<2)) {
                ...
                printf("\n\n status of send input to server %d\n\n", status);
            }
